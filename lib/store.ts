...
export interface Profile {
  ...
  createdAt: string;
  seenBadgeIds?: string[]; // badges already shown to user (for new-badge notification)
}
...
export const ALL_ACTIVITIES: ActivityDef[] = [
  // 0–6 months
  {
    emoji: "🤸", title: "Tummy time",
    image: "https://images.pexels.com/photos/359571/pexels-photo-359571.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Build neck & shoulder strength",
  ...
  {
    emoji: "🎵", title: "Sing lullabies",
    image: "https://images.pexels.com/photos/7491261/pexels-photo-7491261.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Soothes and builds language",
  ...
  {
    emoji: "👀", title: "Track bright toys",
    image: "https://images.pexels.com/photos/10190040/pexels-photo-10190040.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Strengthens visual tracking",
  ...
  {
    emoji: "🔔", title: "Sound matching",
    image: "https://images.pexels.com/photos/8504276/pexels-photo-8504276.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Match sounds to objects or animals",
  ...
  {
    emoji: "🤲", title: "Gentle massage",
    image: "https://images.pexels.com/photos/7282482/pexels-photo-7282482.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Skin-to-skin bonding & body awareness",
  ...
  },
  {
    ...
    image: "https://images.pexels.com/photos/3890481/pexels-photo-3890481.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Read 2 short books today",
  ...
  {
    emoji: "🫶", title: "Emotion cards",
    image: "https://images.pexels.com/photos/5103918/pexels-photo-5103918.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Name feelings from picture cards",
  ...
  {
    emoji: "🧩", title: "Simple puzzles",
    image: "https://images.pexels.com/photos/3933039/pexels-photo-3933039.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Builds problem-solving skills",
  ...
  {
    emoji: "🌱", title: "Mini garden",
    image: "https://images.pexels.com/photos/7782160/pexels-photo-7782160.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Plant seeds and watch them grow",
  ...
  {
    emoji: "✂️", title: "Cutting practice",
    image: "https://images.pexels.com/photos/8612992/pexels-photo-8612992.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Snip paper with child-safe scissors",
  ...
  // 42–84 months
  {
    emoji: "🔡", title: "Letter hunt",
    image: "https://images.pexels.com/photos/7334320/pexels-photo-7334320.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Spot letters on signs & books",
  ...
  {
    emoji: "📖", title: "Reading together",
    image: "https://images.pexels.com/photos/7105613/pexels-photo-7105613.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Read a chapter book aloud",
  ...
  {
    emoji: "🧮", title: "Simple maths games",
    image: "https://images.pexels.com/photos/5412226/pexels-photo-5412226.jpeg?auto=compress&cs=tinysrgb&w=400&h=400&fit=crop", desc: "Add and subtract with objects",
...
/** Every day (YYYY-MM-DD) where a progress update OR journal entry was logged */
export function getActiveDays(profile: Profile): string[] {
  const days = new Set<string>();
  for (const activity of profile.board ?? []) {
    for (const entry of activity.progressLog ?? []) {
...
export function getWeekActiveDays(activeDays: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (s <= todayStr && activeDays.includes(s)) result.push(s);
  }
  return result;
}

/** All 7 calendar dates for the current Mon–Sun week */
...
const BADGE_DEFS: {
  ...
  desc: string;
  tier: "bronze" | "silver" | "gold";
  check: (p: Profile, days: string[]) => boolean;
  progress?: (p: Profile, days: string[]) => { current: number; total: number };
}[] = [
  {
    id: "first_step",
    name: "First Step",
    ...
    tier: "bronze",
    check: (_p, days) => days.length >= 1,
  },
  {
    id: "explorer",
    name: "Explorer",
    emoji: "🗺️",
    desc: "Tried activities in 3 different categories",
    tier: "bronze",
    check: (p) =>
      new Set(
        (p.board ?? []).filter(a => a.progressLog.length > 0).map(a => a.tag)
      ).size >= 3,
    progress: (p) => ({
      current: Math.min(
        new Set((p.board ?? []).filter(a => a.progressLog.length > 0).map(a => a.tag)).size,
        3
      ),
      total: 3,
    }),
  },
  {
    ...
    name: "Memory Maker",
    emoji: "📝",
    desc: "Wrote 3 journal entries",
    tier: "bronze",
    check: (p) => (p.journal ?? []).length >= 3,
    progress: (p) => ({ current: Math.min((p.journal ?? []).length, 3), total: 3 }),
  },
  {
    id: "streak_3",
    name: "3-Day Streak",
    ...
    tier: "silver",
    check: (_p, days) => getCurrentStreak(days) >= 3,
    progress: (_p, days) => ({ current: Math.min(getCurrentStreak(days), 3), total: 3 }),
  },
  {
    id: "active_week",
    name: "Active Week",
    emoji: "📅",
    desc: "Active on 5 days this week",
    tier: "silver",
    check: (_p, days) => getWeekActiveDays(days).length >= 5,
    progress: (_p, days) => ({ current: getWeekActiveDays(days).length, total: 5 }),
  },
  {
    id: "five_done",
    name: "5 Activities Done",
    emoji: "⭐",
    desc: "Completed 5 activities",
    tier: "silver",
    check: (p) => (p.board ?? []).filter(a => a.status === "done").length >= 5,
    progress: (p) => ({
      current: Math.min((p.board ?? []).filter(a => a.status === "done").length, 5),
      total: 5,
    }),
  },
  {
    ...
    tier: "gold",
    check: (_p, days) => getCurrentStreak(days) >= 7,
    progress: (_p, days) => ({ current: Math.min(getCurrentStreak(days), 7), total: 7 }),
  },
  {
    id: "perfect_week",
    name: "Perfect Week",
    emoji: "🏆",
    desc: "Active every day this week",
    tier: "gold",
    check: (_p, days) => getWeekActiveDays(days).length >= 7,
    progress: (_p, days) => ({ current: getWeekActiveDays(days).length, total: 7 }),
  },
  {
    id: "monthly_mover",
    name: "Monthly Mover",
    emoji: "🌟",
    desc: "Active on 15 days this month",
    tier: "gold",
    check: (_p, days) => {
      const start = new Date();
      start.setDate(1);
      const ms = start.toISOString().slice(0, 10);
      return days.filter(d => d >= ms).length >= 15;
    },
    progress: (_p, days) => {
      const start = new Date();
      start.setDate(1);
      const ms = start.toISOString().slice(0, 10);
      return { current: Math.min(days.filter(d => d >= ms).length, 15), total: 15 };
    },
  },
  {
    id: "streak_14",
    ...
    emoji: "👑",
    desc: "Active 14 days in a row",
    tier: "gold",
    check: (_p, days) => getCurrentStreak(days) >= 14,
    progress: (_p, days) => ({ current: Math.min(getCurrentStreak(days), 14), total: 14 }),
  },
];

export function getBadges(profile: Profile): Badge[] {
  ...
  return BADGE_DEFS.map(def => ({
    ...
    desc: def.desc,
    tier: def.tier,
    earned: def.check(profile, activeDays),
    progress: def.progress ? def.progress(profile, activeDays) : undefined,
  }));
}

/** Returns badges earned since last time user saw them */
export function getNewBadges(profile: Profile): Badge[] {
  const all = getBadges(profile);
  const seen = new Set(profile.seenBadgeIds ?? []);
  return all.filter(b => b.earned && !seen.has(b.id));
}

/** Next badge user is closest to earning (not yet earned, has progress) */
export function getNextBadge(profile: Profile): Badge | null {
  const all = getBadges(profile);
  const candidates = all.filter(b => !b.earned && b.progress);
  if (!candidates.length) return null;
  return candidates.reduce((best, b) => {
    const bRatio = b.progress!.current / b.progress!.total;
    const bestRatio = best.progress!.current / best.progress!.total;
    return bRatio > bestRatio ? b : best;
  });
}
